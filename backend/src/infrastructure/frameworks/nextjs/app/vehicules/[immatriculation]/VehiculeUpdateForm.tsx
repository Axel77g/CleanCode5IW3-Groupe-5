...
export default function VehiculeUpdateForm({vehicule}: { vehicule: VehiculeUpdateFormProps }) {
    ...
    const vehiculePath = `/vehicules/${vehicule.immatriculation}`;
    return (
        <Form state={state} action={formAction} title={"Modifier un véhicule"}>
            ...
            <div className="grid grid-cols-3 gap-6">
                <div>
                    ...
                    <Input type="text" name={"model"} value={vehicule?.model} label={"Modèle du véhicule"}
                    ...
                    <Input type="text" name="year" value={vehicule?.year} label={"Année"}
                           placeholder={"Année du véhicule"} disabled/>
                    <Input type="text" name="vin" value={vehicule?.vin} label="Vin" placeholder="Vin du véhicule"
                           disabled/>
                    <Input type="number" name="mileage" value={state?.mileage ?? vehicule?.mileage}
                ...
                <div>
                    ...
                    <Input type="text" name="maintenanceInterval.mileage"
                           value={vehicule?.maintenanceInterval?.mileage ?? ''}
                           label={"Intervalle de maintenances (km)"} placeholder={"Intervalle de maintenances"}
                           disabled={!vehicule?.maintenanceInterval?.mileage}/>
                    <Input type="text" name="maintenanceInterval.duration"
                           value={vehicule?.maintenanceInterval?.duration ?? ''}
                           label={"Intervalle de maintenances (mois)"} placeholder={"Intervalle de maintenances"}
                           disabled={!vehicule?.maintenanceInterval?.duration}/>
                    <Input type="date" name="maintenanceInterval.lastMaintenance.date"
                           value={vehicule?.maintenanceInterval?.lastMaintenance?.date ? vehicule.maintenanceInterval.lastMaintenance.date.toISOString().split('T')[0] : ''}
                    ...
                    <Input type="number" name="maintenanceInterval.lastMaintenance.mileage"
                           value={vehicule?.maintenanceInterval?.lastMaintenance?.mileage ?? ''}
                ...
                <div>
                    ...
                    <Input type="date" name="warranty.periodStart"
                           value={vehicule?.warranty?.periodStart ? vehicule.warranty.periodStart.toISOString().split('T')[0] : ''}
                           label={"Début de la garantie"} placeholder={"Début de la garantie"} disabled/>
                    <Input type="date" name="warranty.periodEnd"
                           value={vehicule?.warranty?.periodEnd ? vehicule.warranty.periodEnd.toISOString().split('T')[0] : ''}
            ...
            <div className={"flex items-center gap-4"}>
                ...
                <Link href={`${vehiculePath}/maintenances`}>
                    <Button>

...
export default function CustomerRegisterForm(){
    const [state, formAction] = useActionState<ActionState,FormData>(registerCustomer,initialState)

    return (
        <Form action={formAction} title={"Ajouter un client"} state={state}>
            <Input type="text" label={"Nom"} name="name" placeholder={"Nom du client"} value={state.name}/>
            <Input placeholder={"Téléphone"} label={"Téléphone"} name={"phoneNumber"} type={"phone"} value={state.phoneNumber}/>
            <Input placeholder={"Email"} label={"Email"} name={"email"} type={"email"} value={state.email}/>
            <div>
                <hr/>
                <br/>
                <h2>Adresse</h2>

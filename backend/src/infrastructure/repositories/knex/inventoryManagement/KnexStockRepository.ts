...
export class KnexStockRepository extends AbstractKnexRepository implements StockRepository {
    ...
    async add(sparePart: InventorySparePart, siret: Siret, quantity: number): Promise<VoidResult> {
    ...
    async getStock(siret: Siret): Promise<Result<StockInventorySparePart[]>> {
        try {
            const allTransactions = await this.getQuery().where('dealer_siret', siret.getValue()) as any[];
    ...
    async getStockQuantity(sparePart: InventorySparePart, siret: Siret): Promise<Result<number>> {
        try {
            const stockTransactionsForSparePart = await this.getQuery()
                ...
                .where('dealer_siret', siret.getValue())
    ...
    async remove(sparePart: InventorySparePart, siret: Siret, quantity: number): Promise<VoidResult> {
        const transaction = await this.connection.transaction();
        try {
            await transaction(this.tableName).insert({
                spare_part_reference: sparePart.reference,
                dealer_siret: siret.getValue(),
                quantity: -quantity
            });
            await transaction.commit();
            return Result.SuccessVoid();

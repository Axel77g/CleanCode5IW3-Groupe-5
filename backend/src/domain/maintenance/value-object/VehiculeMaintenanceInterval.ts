...
export class VehiculeMaintenanceInterval{
    ...
    static create(duration: number, mile : number, lastMaintenance : {date:Date, mileage: number}): VehiculeMaintenanceInterval | ApplicationException {
    ...
    needMaintenance() : boolean {
        return this.lastMaintenance.mileage + this.mileage <= this.mileage || this.lastMaintenance.date.getTime() + this.duration <= Date.now();
    }
}
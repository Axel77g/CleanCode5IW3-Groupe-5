...
export class VehiculeMaintenanceInterval {
    private constructor(
        ...
        public readonly lastMaintenance?: { date: Date; mileage: number }
    ) {}
    ...
    static create(
        duration: number,
        mile: number,
        lastMaintenance?: { date: Date; mileage: number }
    ...
    needMaintenance(): boolean {
        return (
            (this.lastMaintenance?.mileage ?? 0) + this.mileage <= this.mileage ||
            (this.lastMaintenance?.date?.getTime() ?? 0) + this.duration <= Date.now()
        );
    }
}
